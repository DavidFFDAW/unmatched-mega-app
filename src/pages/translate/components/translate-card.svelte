...
<div class="flex center acenter box no-bg">
	<div id="unmatchedCard">
		<div class="unmatched card bgimg" style="background-image: url({cardData.image});">
			...
			<div
				id="unmatchedCardInner"
				class="inner"
				style="width: {cardData.effectSpaceWidth}%; left: {cardData.effectSpaceLeft}px"
			>
				<svg
			...
			<div class="inner inner-texts" id="unmatchedCardInnerTexts">
				...
				<section class="card-real-effects" id="unmatchedCardRealEffectsContainer">
					{#if !cardData.rayito && cardData.inmediate}
						...
						>
							<p>
								...
								<span id="unmatchedCardInmediatelyText" class="prewrap"
									>{cardData.inmediate.replace(/potenciador/g, 'POTENCIADOR')}.</span
								>
							</p>
						</div>
					{/if}
					...
					{#if !cardData.rayito && cardData.during}
						<div class:active={cardData.during} class="real-effect during" id="unmatchedCardDuring">
							<p>
								...
								<span id="unmatchedCardDuringText" class="prewrap"
									>{cardData.during.replace(/potenciador/g, 'POTENCIADOR')}.</span
								>
							</p>
						</div>
					{/if}
					...
					{#if !cardData.rayito && cardData.after}
						<div class:active={cardData.after} class="real-effect after" id="unmatchedCardAfter">
							<p>
								...
								<span id="unmatchedCardAfterText" class="prewrap"
									>{cardData.after.replace(/potenciador/g, 'POTENCIADOR')}.</span
								>
							</p>
						</div>
					{/if}
					...
					{#if cardData.rayito}
						<div class:active={cardData.rayito} class="real-effect rayito" id="unmatchedCardRayito">
							<p>
								<span id="unmatchedCardRayitoText" class="prewrap"
									>{cardData.rayito.replace(/potenciador/g, 'POTENCIADOR')}.</span
								>
							</p>
						</div>
					{/if}
				...
				<div data-v-d8d5fac2="" class="inner-bottom">
					<div data-v-d8d5fac2="" class="bottom-right">
						<span data-v-d8d5fac2="" class="deck-name">{cardData.name}</span><span
							data-v-d8d5fac2=""
							class="card-quantity"
						>
							x{cardData.qty}
				...
				<!-- <footer class="card-deck-footer">
					<p style="white-space: nowrap; font-size: 8px; letter-spacing: 0;">
						<span
							id="unmatchedCardDeckName"
							class="league name"
							style="font-size: 8px; letter-spacing: 0px;"
						>
							{cardData.name}
						</span>
						|
						<span
							id="unmatchedCardQuantity"
							class="league number-quantity"
...
<style>
	...
	.inner-bottom .bottom-right .deck-name {
	...
	.unmatched.card .inner .card-deck-footer .name span {
		text-transform: uppercase;
	}
	.unmatched.card .inner .card-deck-footer .number-quantity span {
		font-family: 'league', sans-serif;
	}
	.unmatched.card .inner .card-deck-footer p {
		letter-spacing: 1px;
